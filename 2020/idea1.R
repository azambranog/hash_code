...
doall <- function(f) {
  ...
  libs <- data[, .(med_score = median(score), sign_time=min(sign), bpd=min(bpd), tot_books= .N), by = lib]
  ...
  libs[, rank := 1.3*mx + 1.1*sx + 1.1*bx + 1*dx]
  ...
  writeLines(res, file.path('clean', paste0('idea1_', f, '.txt')))
